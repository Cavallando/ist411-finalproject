...
class CustomNav extends Component {
    ...
    render() {
        ...
        const showLoginButton = (!isAuthenticated() ? (<NavItem eventKey={1} onClick={() => this.props.auth.login()}>Login/Sign up</NavItem>) : (<NavDropdown onSelect={this.onSelect} eventKey={5} title={<span><img alt="Profile" className="nav-icon" title="Profile" src={profileIcon}></img>  {this.props.profile.name}</span>} id="basic-nav-dropdown"><MenuItem eventKey={5.1}>My Profile</MenuItem><MenuItem eventKey={5.2}>My Paintings</MenuItem><MenuItem eventKey={5.3}>Logout</MenuItem></NavDropdown>))
        var showMyPaintings = [];
